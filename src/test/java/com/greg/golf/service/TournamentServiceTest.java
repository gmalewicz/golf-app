...
class TournamentServiceTest {
	...
	void calculateNetAndGrossSTBTest(@Autowired RoundRepository roundRepository,
        ...
        log.debug("STB net: {}", tournamentResult.getStbNet());
        log.debug("STB gross: {}", tournamentResult.getStbGross());
	...
	void calculateCorrectedStrokesTest(@Autowired HoleRepository holeRepository, @Autowired PlayerService playerService,
        ...
        log.debug("corrected Strokes: {}", correctedScore);
	...
	void calculateScoreDifferentialTest(@Autowired HoleRepository holeRepository, @Autowired PlayerService playerService,
        ...
        log.debug("score differential: {}", scoreDifferential);
	...
	@DisplayName("Attempt to update tournament player handicap for tournament which does not allow for that")
	@Transactional
	@Test
	void attemptToUpdateTournamentPlayerHcpNotAllowedTest(@Autowired TournamentRepository tournamentRepository,
										   @Autowired TournamentPlayerRepository tournamentPlayerRepository,
												@Autowired PlayerService playerService) {
		...
		var tournamentId = tournamentPlayer.getTournamentId();
		var playerId = tournamentPlayer.getPlayerId();

		assertThrows(HcpChangeNotAllowedException.class, () ->
				tournamentService.updatePlayerHcp(tournamentId, playerId, 2F));

	}

	@DisplayName("Attempt to update tournament player handicap for tournament which allow for that")
	@Transactional
	@Test
	void attemptToUpdateTournamentPlayerHcpAllowedTest(@Autowired TournamentRepository tournamentRepository,
														  @Autowired TournamentPlayerRepository tournamentPlayerRepository,
														  @Autowired PlayerService playerService) {
		...
		t2.setEndDate(new Date(1));
		t2.setStartDate(new Date(1));
		t2.setName("Test Cup");
		t2.setPlayer(player);
		t2.setStatus(Tournament.STATUS_OPEN);
		t2.setBestRounds(Common.ALL_ROUNDS);
		t2.setMaxPlayHcp(54);
		t2.setPlayHcpMultiplayer(1f);
		t2.setCanUpdateHcp(true);
		t2 = tournamentRepository.save(t2);

		var tournamentPlayer = new TournamentPlayer();
		tournamentPlayer.setPlayerId(1L);
		tournamentPlayer.setTournamentId(t2.getId());
		tournamentPlayer.setNick("Test");
		tournamentPlayer.setWhs(1F);
		tournamentPlayerRepository.save(tournamentPlayer);
		...
		assertEquals(2F, tournamentService.getTournamentPlayers(t2.getId()).getFirst().getWhs());
	}
	...
	void attemptToDeleteTeeTimesByAuthorizedUserTest(@Autowired PlayerService playerService) {
		...
		var teeTimeParameters = getTeeTimeParameters();
	...
	private static @NotNull TeeTimeParameters getTeeTimeParameters() {
		var teeTimeParameters = new TeeTimeParameters();
		teeTimeParameters.setFirstTeeTime("10:00");
		teeTimeParameters.setFlightSize(4);
		teeTimeParameters.setPublished(false);
